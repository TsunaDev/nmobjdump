...
** nmobjdump
** File description:
** 32 bits architecture flags management for file infos
*/
...
static size_t	has_reloc(void *data, elf32_t *elf, char **str)
{
	char	*shstrtab = data +
		elf->sections[elf->header->e_shstrndx].sh_offset;

	for (size_t idx = 0; idx < elf->header->e_shnum; idx++) {
		if (elf->sections[idx].sh_type == SHT_REL &&
		strcmp(&shstrtab[elf->sections[idx].sh_name], ".rel.dyn") &&
		strcmp(&shstrtab[elf->sections[idx].sh_name], ".rel.plt")) {
...
static size_t	has_syms(elf32_t *elf, char **str)
{
	for (size_t idx = 0; idx < elf->header->e_shnum; idx++) {
...
size_t	get_flags32(void *data, elf32_t *elf, char **str)
{
	size_t	flags = 0x00;
